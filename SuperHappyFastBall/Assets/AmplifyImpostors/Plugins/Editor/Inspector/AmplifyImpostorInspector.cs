...
namespace AmplifyImpostors
{
	...
	public sealed class AmplifyImpostorInspector : Editor
	{
		...
		private const string TextureSizeStr = "Texture Size";
		private const string AxisFramesStr = "Axis Frames";
		...
		private void OnEnable()
		{
			...
			//if( ImpostorBakingTools.GlobalDefault && !m_instance.m_overrideGlobalDefault )
			//	m_instance.m_folderMode = FolderMode.Global;
		...
		override public void OnInspectorGUI()
		{
			...
			m_instance.Data = EditorGUILayout.ObjectField("Impostor Asset", m_instance.Data, typeof( AmplifyImpostorAsset ), false ) as AmplifyImpostorAsset;
			if( m_instance.Data != null )
				m_currentData = m_instance.Data;

			m_instance.LodGroup = EditorGUILayout.ObjectField( "LOD Group", m_instance.LodGroup, typeof( LODGroup ), true ) as LODGroup;

			EditorGUILayout.BeginHorizontal();
			Color tempC = GUI.color;
			GUI.color = m_flash;
			EditorGUILayout.PropertyField( m_renderers, new GUIContent( "References" ), true );
			GUI.color = tempC;
			...
			if( ImpostorBakingTools.GlobalBakingOptions && m_instance.Data != null)
			{
				...
				{
					EditorGUI.BeginChangeCheck();
					m_currentData.ImpostorType = (ImpostorType)EditorGUILayout.EnumPopup( "Bake Type", m_currentData.ImpostorType );
					if( EditorGUI.EndChangeCheck() )
					{
						m_recalculatePreviewTexture = true;
					...
					m_currentData.PixelPadding = EditorGUILayout.IntSlider( "Pixel Padding", m_currentData.PixelPadding, 0, 64 );
#if UNITY_2017_3_OR_NEWER
					m_currentData.BufferMask = (DeferredBuffers)EditorGUILayout.EnumFlagsField( "Rendering Maps", m_currentData.BufferMask );
#else
					m_currentData.BufferMask = (DeferredBuffers)EditorGUILayout.EnumMaskField( "Rendering Maps", m_currentData.BufferMask );
#endif
					EditorGUI.BeginDisabledGroup( m_instance.m_lastImpostor != null || m_instance.LodGroup == null );
					EditorGUI.BeginChangeCheck();
					m_instance.m_lodReplacement = (LODReplacement)EditorGUILayout.EnumPopup( "LOD Insert Mode", m_instance.m_lodReplacement );
					EditorGUI.BeginDisabledGroup( m_instance.m_lodReplacement < LODReplacement.ReplaceSpecific || m_instance.LodGroup == null );
					{
						...
						m_instance.m_insertIndex = EditorGUILayout.IntSlider( "LOD Target Index", m_instance.m_insertIndex, 0, maxLods );
					}
					EditorGUI.EndDisabledGroup();
					if( EditorGUI.EndChangeCheck() /*|| m_instance.LodGroup.lodCount !=*/ )
					{
						ReCheckRenderers();
					}
					EditorGUI.EndDisabledGroup();
						...
						EditorGUILayout.BeginVertical();
						{
							...
							switch( m_instance.m_cutMode )
							{
								...
								case CutMode.Automatic:
								{
									EditorGUI.BeginChangeCheck();
									{
										m_currentData.MaxVertices = EditorGUILayout.IntSlider( "Max Vertices", m_currentData.MaxVertices, 4, 16 );
										m_currentData.Tolerance = EditorGUILayout.Slider( "Outline Tolerance", m_currentData.Tolerance * 5, 0, 1f ) * 0.2f;
										m_currentData.NormalScale = EditorGUILayout.Slider( "Normal Scale", m_currentData.NormalScale, 0, 1.0f );
									}
									if( EditorGUI.EndChangeCheck() )
									{
										m_recalculateMesh = true;
