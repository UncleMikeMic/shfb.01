...
namespace AmplifyShaderEditor
{
	...
	public sealed class AmplifyImpostorNode : ParentNode
	{
		...
		private void GenerateSphereImpostorVertex()
		{
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "if( unity_LightShadowBias.y == 0 )" );
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "worldCameraPos = UnityWorldSpaceLightDir(mul(unity_ObjectToWorld, v.vertex).xyz) * -5000;" );
			IOUtils.AddFunctionLine( ref m_functionBody, "#else" );
		...
		private void GenerateImpostorVertex()
		{
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "float3 octa1WorldX = normalize( cross( upVector , octa1WorldY ) );" );
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "float3 octa2WorldX = normalize( cross( upVector , octa2WorldY ) );" );
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "float3 octa3WorldX = normalize( cross( upVector , octa3WorldY ) );" );
		...
		void GenerateImpostorFragment( bool withSpec = true, bool withEmission = true )
		{
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "float2 asd = 1 - fraction;" );
			IOUtils.AddFunctionLine( ref m_functionBody, "float dota = dot( fraction , float2( 1,-1 ) );" );
			IOUtils.AddFunctionLine( ref m_functionBody, "float3 weights = float3( min(asd.x, asd.y), abs(dota), min(fraction.x, fraction.y) );" );
